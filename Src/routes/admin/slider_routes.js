...
const { decompose, changeStatus, details, update, view, create } = require('../../controllers/admin/slider_controller');
...
const uploads = multer({ dest: 'uploads/slider'}) // here we set destination of image where we want to upload image.
...
module.exports = server => {
    const storage = multer.diskStorage({  // this function is work to save image in catetory folder
        destination: function (req, file, cb) {
          cb(null, 'uploads/slider') // it means where we want to upload image
        },
...
server.use('/api/admin/slider', router) // this is common route of this file
}
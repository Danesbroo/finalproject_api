...
const { decompose, changeStatus, details, update, view, create, productDetails } = require('../../controllers/admin/products_controller');
...
const uploads = multer({ dest: 'uploads/products'}) // here we set destination of image where we want to upload image.
...
module.exports = server => {
    const storage = multer.diskStorage({  // this function is work to save image in catetory folder
        destination: function (req, file, cb) {
          cb(null, 'uploads/products') // it means where we want to upload image
        },
...
router.post('/create',singleMultiple, create) // here we use upload as a middleware, which gives promission to create create text file in form data file. we can create file without upload image. frontend developer can insert data from in any way we have to insert it and read data we use upload.none() function.
router.post('/view',upload.none(), view )

router.put('/update/:id',singleMultiple, update) // use to add id in url
...
server.use('/api/admin/products', router) // this is common route of this file
}
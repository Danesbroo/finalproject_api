...
const uploads = multer({ dest: 'uploads/admins'}) // here we set destination of image where we want to upload image.
...
const {login, register,forgetPassword, viewProfile, updateProfile, changePassword, resetPassword} = require('../../controllers/admin/admins_controller');
...
module.exports = server => {
    const storage = multer.diskStorage({  // this function is work to save image in catetory folder
        destination: function (req, file, cb) {
          cb(null, 'uploads/admins') // it means where we want to upload image
        },
...
server.use('/api/admin/admins', router) // this is common route of this file
}
...
exports.create = async (request, response) => {
    try {
        ...
        const insertData = new FaqModel(request.body);
        await insertData.save()
            .then((result) => {
                var output = {
                    _status: true,
                    _message: 'FAQ created Sucessfully',
                    _data: result,
                }
...
exports.details = async (request, response) => {
    ...
    //     _id : request.body.id
    // })
    await FaqModel.findById(request.body.id) // this is the alternative method of details function special use to find specific product which user select
        .then((result) => {
            if (result) {
                var output = {
                    _status: true,

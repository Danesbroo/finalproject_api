...
exports.create = async (request, response) => {
    ...
    try {
        const insertData = new whyChooseUsModel(saveData);
...
exports.view = async (request, response) => {
    try {
        ...
        const totalRecords = await whyChooseUsModel.countDocuments(filter);
        const totalPage = Math.ceil(totalRecords / limit);

        const result = await whyChooseUsModel.find(filter)
            .skip(skip)
            .limit(limit)
        ...
        if (result.length > 0) {
            response.send({
                ...
                _image_path: process.env.WHY_CHOOSE_US_IMAGE,
                _data: result
            });
        } else {
            response.send({
...
exports.details = async (request, response) => {
    await whyChooseUsModel.findById(request.body.id)
        .then((result) => {
            response.send({
                _status: true,
...
exports.changeStatus = async (request, response) => {
    await whyChooseUsModel.updateMany(
